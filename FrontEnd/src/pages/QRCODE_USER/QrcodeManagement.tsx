...
export default function QrcodeManagement() {
  ...
  const handleSubmitEdit = () => {
    ...
    // 1. Validation tên khách
    if (!editForm.visitor_name?.trim()) {
      newErrors.visitor_name = 'Vui lòng nhập tên khách'
      hasError = true
    }

    // 2. Validation số điện thoại (BẮT BUỘC)
    if (!editForm.visitor_phone?.trim()) {
      newErrors.visitor_phone = 'Vui lòng nhập số điện thoại'
      hasError = true
    } else {
      const phone = editForm.visitor_phone.trim()
      const phoneRegex = /^(0[0-9]{9}|84[0-9]{9}|\+84[0-9]{9})$/
      if (!phoneRegex.test(phone)) {
        newErrors.visitor_phone = 'Số điện thoại không hợp lệ (VD: 0912345678)'
        hasError = true
      }
    }

    // 3. Validation CCCD (BẮT BUỘC)
    if (!editForm.visitor_id_card?.trim()) {
      newErrors.visitor_id_card = 'Vui lòng nhập CCCD/CMND'
      hasError = true
    } else {
      const idCard = editForm.visitor_id_card.replace(/\s/g, '')
      const idRegex = /^[0-9]{9}$|^[0-9]{12}$/
      if (!idRegex.test(idCard)) {
        newErrors.visitor_id_card = 'CCCD/CMND phải có 9 hoặc 12 chữ số'
        hasError = true
      }
    }

    // 4. Validation số lượt tối đa >= 1
    if (!editForm.max_entries || editForm.max_entries < 1) {
      newErrors.max_entries = 'Số lượt tối đa phải lớn hơn hoặc bằng 1'
      hasError = true
    }

    // 5. Validation ngày hết hạn không được là ngày quá khứ
    if (editForm.valid_to) {
      const expiryDate = new Date(editForm.valid_to)
      const today = new Date()
      today.setHours(0, 0, 0, 0)

      if (expiryDate < today) {
        newErrors.valid_to = 'Ngày hết hạn không được là ngày trong quá khứ'
        hasError = true
      }
    } else {
      newErrors.valid_to = 'Vui lòng chọn ngày hết hạn'
      hasError = true
    }
        ...
        <div className='fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn'>
          <div className='bg-surface-container-lowest rounded-2xl max-w-md w-full shadow-2xl animate-slideUp'>
            ...
            <div className='p-6 space-y-4'>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setErrors({ ...errors, visitor_name: '' })
                  }}
                  className={`w-full px-4 py-2.5 bg-surface-container-low border rounded-xl focus:outline-none focus:border-primary transition-all ${
                    errors.visitor_name ? 'border-error' : 'border-outline-variant/20'
                  }`}
                  placeholder='Nhập tên khách'
                />
                {errors.visitor_name && (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Không giới hạn số lượng, để user nhập tự do
                    setEditForm({ ...editForm, visitor_phone: value })
                    setErrors({ ...errors, visitor_phone: '' })
                  }}
                  className={`w-full px-4 py-2.5 bg-surface-container-low border rounded-xl focus:outline-none focus:border-primary transition-all ${
                    errors.visitor_phone ? 'border-error' : 'border-outline-variant/20'
                  }`}
              ...
              <div>
                <label className='block text-xs font-bold text-on-surface uppercase tracking-wider mb-2'>
                  CCCD/CMND <span className='text-error'>*</span>
                </label>
                <input
                  type='text'
                  value={editForm.visitor_id_card}
                  onChange={(e) => {
                    ...
                    // Không giới hạn độ dài khi nhập, chỉ validate khi submit
                    setEditForm({ ...editForm, visitor_id_card: value })
                    setErrors({ ...errors, visitor_id_card: '' })
                  }}
                  className={`w-full px-4 py-2.5 bg-surface-container-low border rounded-xl focus:outline-none focus:border-primary transition-all font-mono ${
                    errors.visitor_id_card ? 'border-error' : 'border-outline-variant/20'
                  }`}
                  placeholder='123456789/012345678912'
                />
                {errors.visitor_id_card && (
                  <p className='text-xs text-error mt-1 flex items-center gap-1'>
                    <span className='material-symbols-outlined text-xs'>error</span>
                    {errors.visitor_id_card}
                  </p>
              ...
              {/* <div>
                <label className='block text-xs font-bold text-on-surface uppercase tracking-wider mb-2'>
                  CCCD <span className='text-error'>(12 số)</span>
                </label>
                <input
                  type='text'
                  value={editForm.visitor_id_card}
              ...
              <div>
                <label className='block text-xs font-bold text-on-surface uppercase tracking-wider mb-2'>
                  Số lượt tối đa <span className='text-error'>*</span>
                </label>
                <input
                  type='number'
                  value={editForm.max_entries}
                  onChange={(e) => {
                    setEditForm({ ...editForm, max_entries: parseInt(e.target.value) || 1 })
                    setErrors({ ...errors, max_entries: '' })
                  }}
                  className={`w-full px-4 py-2.5 bg-surface-container-low border rounded-xl focus:outline-none focus:border-primary transition-all ${
                    errors.max_entries ? 'border-error' : 'border-outline-variant/20'
                  }`}
                  min={1}
                />
                {errors.max_entries && (
              ...
              <div>
                <label className='block text-xs font-bold text-on-surface uppercase tracking-wider mb-2'>
                  Thời hạn <span className='text-error'>*</span>
                </label>
                <input
                  type='date'
                  value={editForm.valid_to ? new Date(editForm.valid_to).toISOString().slice(0, 10) : ''}
                  onChange={(e) => {
                    setEditForm({ ...editForm, valid_to: new Date(e.target.value).toISOString() })
                    setErrors({ ...errors, valid_to: '' })
                  }}
                  className={`w-full px-4 py-2.5 bg-surface-container-low border rounded-xl focus:outline-none focus:border-primary transition-all ${
                    errors.valid_to ? 'border-error' : 'border-outline-variant/20'
                  }`}
                />
                {errors.valid_to && (
                  <p className='text-xs text-error mt-1 flex items-center gap-1'>
