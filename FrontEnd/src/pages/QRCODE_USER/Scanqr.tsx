...
export default function ScanQrPage() {
  ...
  const [activeTab, setActiveTab] = useState<'scan' | 'upload' | 'manual'>('scan')
  const [cameraActive, setCameraActive] = useState(false)
  const [direction, setDirection] = useState<'IN' | 'OUT'>('IN') // 👈 Thêm state chọn hướng
  const qrCodeRef = useRef<Html5Qrcode | null>(null)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const hasScannedRef = useRef(false)
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      const onScanSuccess = (decodedText: string) => {
        if (!hasScannedRef.current) {
          hasScannedRef.current = true
          toast.success(`Đã quét mã QR thành công! Hướng: ${direction === 'IN' ? 'VÀO' : 'RA'}`)
          stopCamera()
          // CHỈNH SỬA: Chuyển sang trang kết quả kèm theo hướng
          navigate(`/result/${encodeURIComponent(decodedText)}?direction=${direction}`)
        }
      }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await html5QrCode.scanFile(file, true)
      toast.success(`Đọc mã QR từ ảnh thành công! Hướng: ${direction === 'IN' ? 'VÀO' : 'RA'}`)
      navigate(`/result/${encodeURIComponent(result)}?direction=${direction}`)
    } catch (err) {
      toast.error('Không tìm thấy mã QR trong ảnh')
    } finally {
      setUploading(false)
  ...
  const handleManualInput = (e: React.FormEvent) => {
    ...
    const input = form.qrCode as HTMLInputElement
    if (input.value) {
      navigate(`/result/${encodeURIComponent(input.value)}?direction=${direction}`)
    }
  }
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen font-['Inter',sans-serif]">
      ...
      <div className='flex flex-col min-h-screen'>
        {/* Main Content */}
        <main className='flex-1 max-w-7xl mx-auto w-full px-4 md:px-6 py-8'>
          {/* Direction Selection - 2 nút VÀO/RA (giữ nguyên layout cũ) */}
