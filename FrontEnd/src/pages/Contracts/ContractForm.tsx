...
export default function ContractForm({ contractId, isOpen, onClose, currentStatus  }: ContractFormProps) {
  ...
  // Fetch apartments & residents
  useEffect(() => {
    if (isOpen) {
      http.get('/api/apartments/available').then((res) => {
        setApartments(res.data?.data || []);
      });
      http.get('/api/users').then((res) => setResidents(res.data?.data || []));
    }
  }, [isOpen]);
  ...
  useEffect(() => {
    if (isEdit && contractId && isOpen) {
      http.get(`/api/contracts/${contractId}`).then((res) => {
        ...
        if (c) {
          setFormData({
            apartmentId: c.apartment?.id || c.apartmentId || '',
            residentId: c.signer?.id || c.residentId || '',
        ...
        <form onSubmit={handleSubmit} className='flex-1 overflow-y-auto px-8 py-6 space-y-6'>
                ...
                >
                  ...
                  {apartments.map((a: any) => (
                    <option key={a.id} value={a.id}>
                      {a.apartment_code || a.apartmentCode} - {a.building_name || a.buildingName}
                    </option>
                  ))}
                </select>
          ...
          <section>
            ...
            <div className='max-w-sm'>
              <label className='block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5'>
                Cư dân <span className='text-red-400'>*</span>
              </label>
              <input
                type='number'
                ...
                placeholder='Nhập ID cư dân (VD: 1)'
              />
