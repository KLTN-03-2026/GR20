...
export default function QrcodeManagement() {
  ...
  const activeCount = guestQrs.filter((qr: Qrcodes) => qr.isActive).length
  const expiredToday = guestQrs.filter((qr: Qrcodes) => {
    if (!qr.isActive) return false
    const validTo = new Date(qr.validTo)
    const today = new Date()
    return validTo.toDateString() === today.toDateString()
  }).length
      ...
      <div className='pb-20 px-6 lg:px-12 w-full max-w-7xl mx-auto'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-12'>
          ...
          <div className='bg-surface-container-low p-6 rounded-3xl shadow-sm'>
            <p className='text-on-surface-variant text-sm font-medium'>Hết hạn hôm nay</p>
                    ...
                    <tr key={qr.id} className='hover:bg-slate-50 transition-colors group'>
                      ...
                      <td className='px-8 py-6 text-right'>
                        <div className='flex justify-end gap-2'>
                          ...
                          {qr.isActive && !qr.isExpired && !qr.isRevoked && (
                            <button
