...
const getByBuilding = async (req, res) => {
  try {
    const result = await service.getApartmentsByBuilding(Number(req.params.buildingId), req.query);
...
const getByFloor = async (req, res) => {
  try {
    const result = await service.getApartmentsByFloor(Number(req.params.floorId), req.query);
    res.json({ operationType: "Success", message: "success", code: "OK", ...result, timestamp: new Date() });
  } catch (err) {
    sendError(res, err);
  }
...
const getStats = async (_req, res) => {
  try {
    ...
    const expiring = await pool.query(`SELECT COUNT(*) FROM contracts WHERE status = 'ACTIVE' AND end_date <= NOW() + INTERVAL '30 days'`);
    res.json({
      operationType: "Success",
      data: {
