...
const getAllResidentAccessHistory = async (options = {}) => {
  ...
  const offset = (page - 1) * limit;
  let conditions = [`qc.user_id IS NOT NULL`];
  let params = [];
  let paramIndex = 1;

  if (search && search.trim()) {
    conditions.push(`(u.full_name ILIKE $${paramIndex} OR u.phone ILIKE $${paramIndex} OR u.email ILIKE $${paramIndex} OR a.apartment_code ILIKE $${paramIndex})`);
    params.push(`%${search.trim()}%`);
    paramIndex++;
  }
  ...
  // ✅ Tạo countParams riêng
  let countConditions = [`qc.user_id IS NOT NULL`];
  let countParams = [];
  let countParamIndex = 1;

  if (search && search.trim()) {
    countConditions.push(`(u.full_name ILIKE $${countParamIndex} OR u.phone ILIKE $${countParamIndex} OR u.email ILIKE $${countParamIndex} OR a.apartment_code ILIKE $${countParamIndex})`);
...
const getAllPersonalQrs = async (options = {}) => {
  ...
  // Tìm kiếm theo tên, email, phone, apartment_code
  if (search && search.trim()) {
    conditions.push(`(u.full_name ILIKE $${paramIndex} OR u.email ILIKE $${paramIndex} OR u.phone ILIKE $${paramIndex} OR a.apartment_code ILIKE $${paramIndex})`);
    params.push(`%${search.trim()}%`);
    paramIndex++;
  }
